...
class RaycastAnchorVariableShoulder extends RaycastBase_1.default {
    ...
    getVerticalHeadTransform() {
        let levelRotation = quat.lookAt(this.camera.back(), vec3.up());
    ...
    estimateShoulderPosition(handPosition) {
        ...
        let elevationIntervalElbow = MathUtils.clamp((0, mathUtils_1.inverseLerp)(ELBOW_LOCATION.y + ELBOW_TRACKED_RADIUS, ELBOW_LOCATION.y - ELBOW_TRACKED_RADIUS, handElevationHeadset), 0, 1);
        let lateralIntervalElbow = MathUtils.clamp((0, mathUtils_1.inverseLerp)(ELBOW_LOCATION.x + ELBOW_TRACKED_RADIUS_HORIZONTAL, ELBOW_LOCATION.x - ELBOW_TRACKED_RADIUS_HORIZONTAL, handLateralHeadset), 0, 1);
        let lerpInvY = Math.abs(1 - elevationIntervalElbow ** 2);
        let lerpInvX = Math.abs(1 - lateralIntervalElbow ** 2);
        let variableElbowOffset = ELBOW_LOCATION.add(new vec3(Math.cos(lerpInvX * Math.PI * -1) * ELBOW_RADIUS_HORIZONTAL, Math.cos(lerpInvY * Math.PI * -1) * ELBOW_RADIUS, Math.sin(lerpInvX * Math.PI * -1) * ELBOW_RADIUS +
